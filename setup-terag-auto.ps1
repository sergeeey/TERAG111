...
try {
    ...
    if ($javaVersion -match "version \"(\d+)") {
        ...
        if ($javaMajor -ge 17) {
            ...
            Write-Host "⚠️ Java version too old: $javaVersion (need 17+)" -ForegroundColor Yellow
            Write-Host "Installing Java 17..." -ForegroundColor Yellow
            $skipJavaInstall = $false
        }
    } else {
        Write-Host "⚠️ Java not found. Installing Java 17..." -ForegroundColor Yellow
        $skipJavaInstall = $false
    }
} catch {
    Write-Host "⚠️ Java not found. Installing Java 17..." -ForegroundColor Yellow
...
# 2️⃣ Установка Java 17 (если нужно)
if (-not $skipJavaInstall) {
    Write-Host "2️⃣ Installing Java 17..." -ForegroundColor Yellow
    ...
    try {
        ...
        Write-Host "✅ Java 17 MSI downloaded" -ForegroundColor Green
    } catch {
        Write-Host "❌ Failed to download Java 17: $($_.Exception.Message)" -ForegroundColor Red
        exit 1
    }
    ...
    try {
        $javaVersion = java -version 2>&1
        Write-Host "✅ Java verification: $javaVersion" -ForegroundColor Green
    } catch {
...
# 3️⃣ Установка Neo4j (если нужно)
if (-not $SkipNeo4j) {
    Write-Host "3️⃣ Installing Neo4j Community Edition..." -ForegroundColor Yellow
    ...
    if (-not (Test-Path "$neo4jPath\bin\neo4j.bat")) {
        ...
        try {
            ...
            Write-Host "✅ Neo4j downloaded" -ForegroundColor Green
        } catch {
            Write-Host "❌ Failed to download Neo4j: $($_.Exception.Message)" -ForegroundColor Red
            exit 1
        }
        ...
        try {
            ...
            Write-Host "✅ Neo4j extracted" -ForegroundColor Green
        } catch {
            Write-Host "❌ Failed to extract Neo4j: $($_.Exception.Message)" -ForegroundColor Red
            exit 1
        }
    } else {
    ...
    try {
        ...
        Write-Host "✅ Neo4j password set to: 12345" -ForegroundColor Green
    } catch {
        Write-Host "⚠️ Could not set Neo4j password (may already be set)" -ForegroundColor Yellow
    }
    ...
    try {
        ...
        Write-Host "❌ Failed to start Neo4j: $($_.Exception.Message)" -ForegroundColor Red
    }
}
...
# 5️⃣ Создание индексатора с графовой поддержкой
Write-Host "5️⃣ Creating enhanced code indexer..." -ForegroundColor Yellow

# Копируем существующий index_codebase_v2.py если есть
if (Test-Path "index_codebase_v2.py") {
    Write-Host "✅ Enhanced indexer already exists" -ForegroundColor Green
} else {
...
"@
    ...
    Write-Host "✅ Created enhanced indexer" -ForegroundColor Green
}

# 6️⃣ Создание RAG с графовой поддержкой
Write-Host "6️⃣ Creating enhanced RAG..." -ForegroundColor Yellow

if (Test-Path "ask_rag_v2.py") {
    Write-Host "✅ Enhanced RAG already exists" -ForegroundColor Green
} else {
    # Создаём базовый RAG
    $ragCode = @"
import sys
...
"@
    ...
    Write-Host "✅ Created enhanced RAG" -ForegroundColor Green
}

# 7️⃣ Индексация проекта
Write-Host "7️⃣ Indexing project..." -ForegroundColor Yellow
...
# 8️⃣ Создание конфигурации Cursor
Write-Host "8️⃣ Creating Cursor configuration..." -ForegroundColor Yellow
...
Write-Host "✅ Created Cursor configuration" -ForegroundColor Green

# 9️⃣ Создание README
Write-Host "9️⃣ Creating setup documentation..." -ForegroundColor Yellow
...
Write-Host "✅ Created setup documentation" -ForegroundColor Green
...
Write-Host "✅ Java 17 (OpenJDK Temurin)" -ForegroundColor White
Write-Host "✅ Neo4j Community Edition 5.23.0" -ForegroundColor White
Write-Host "✅ Python dependencies" -ForegroundColor White
Write-Host "✅ Enhanced RAG with graph support" -ForegroundColor White
Write-Host "✅ Project indexed and ready" -ForegroundColor White
Write-Host ""
