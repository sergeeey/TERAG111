# TERAG Optimized Setup - Java + Neo4j + Python Integration
# Оптимизированная настройка для существующего Ollama
...
Write-Host "TERAG Optimized Setup (Ollama Ready)" -ForegroundColor Cyan
Write-Host "=====================================" -ForegroundColor Cyan
Write-Host ""

# 1. Проверка Ollama (уже установлен)
Write-Host "1. Verifying Ollama..." -ForegroundColor Yellow
...
try {
    ...
    if ($models -match "deepseek-coder|mistral|qwen") {
        Write-Host "OK: Ollama models available" -ForegroundColor Green
    } else {
        Write-Host "WARNING: No suitable models found. Consider running: ollama pull deepseek-coder" -ForegroundColor Yellow
    }
} catch {
    Write-Host "WARNING: Could not check Ollama models" -ForegroundColor Yellow
}
...
if (-not $skipJavaInstall) {
    Write-Host "4. Installing Java 17..." -ForegroundColor Yellow
...
# 5. Установка Neo4j (если нужно)
if (-not $SkipNeo4j) {
    Write-Host "5. Installing Neo4j Community Edition..." -ForegroundColor Yellow
...
# 7. Создание индексатора с графовой поддержкой
Write-Host "7. Creating enhanced code indexer..." -ForegroundColor Yellow
...
# 8. Создание RAG с графовой поддержкой
Write-Host "8. Creating enhanced RAG..." -ForegroundColor Yellow
...
# 9. Индексация проекта
Write-Host "9. Indexing project..." -ForegroundColor Yellow
...
# 10. Создание конфигурации Cursor
Write-Host "10. Creating Cursor configuration..." -ForegroundColor Yellow
...
$cursorConfig | ConvertTo-Json -Depth 3 | Out-File -FilePath "cursor_config_optimized.json" -Encoding UTF8
Write-Host "OK: Created Cursor configuration" -ForegroundColor Green

# 11. Создание README
Write-Host "11. Creating setup documentation..." -ForegroundColor Yellow

$readmeContent = @"
# TERAG Optimized Setup - COMPLETE!

## What's Installed

- Ollama: Already running (verified)
...
---
TERAG Optimized Setup - Production Ready!
"@

$readmeContent | Out-File -FilePath "TERAG_OPTIMIZED_SETUP.md" -Encoding UTF8
Write-Host "OK: Created setup documentation" -ForegroundColor Green

Write-Host ""
Write-Host "TERAG OPTIMIZED SETUP FINISHED!" -ForegroundColor Green
Write-Host "==================================" -ForegroundColor Green
Write-Host ""
Write-Host "Installed components:" -ForegroundColor Cyan
Write-Host "OK: Ollama (already running)" -ForegroundColor White
